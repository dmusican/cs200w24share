// Chess board with queens where none threatens another.
public class Queens
{
  ...
  public static boolean safe(char[][] board, int row, int col)
  {
    ...
    for (i=row, j=col; ___________________________________)
      if (board[i][j] == 'Q')
        return false;

    // Check diagonal up and right
    for (i=row, j=col; ____________________________________)
      if (board[i][j] == 'Q')
        return false;

    // Check diagonal down and left
    for (i=row, j=col; _____________________________________)
      if (board[i][j] == 'Q')
        return false;
  ...
  public static char[][] completeBoard(char[][] board)
  {
    ...
    for (int i=0; i < SIZE; i++)
      for (int j=0; j < SIZE; j++)
        if (safe(board,i,j))
          {
            char[][] copy = copyBoard(__________________);
            copy[i][j] = 'Q';
            char[][] result = completeBoard(________________);
            if (result != null)
